...
local autoParryFireCooldown = 0.02
...
Players.PlayerRemoving:Connect(
    function(plr)
        if plr.Character then
            activeAttack[plr.Character] = nil
        end
    end
)
...
local function checkKillerAttackingInParryRange()
    ...
    for char, _ in pairs(activeAttack) do
...
makeInfo("Combat: Anti-Ragdoll + Auto Parry (Instant)", cb)
makeToggle(
    "Anti Ragdoll",
    false,
    function(state)
...
print("[vanz] Slim menu + Instant Auto Parry")
