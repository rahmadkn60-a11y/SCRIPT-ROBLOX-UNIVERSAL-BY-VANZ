...
do
    ...
    local function createESPTag(plr)
        ...
        bb.AlwaysOnTop = true
        bb.Parent = char:FindFirstChild("Head") or hrp
        ...
        espData[plr] = {box = box, hl = hl, bb = bb}
    end
    ...
    local function startAimbot()
        ...
        aimbotConn = RunService.RenderStepped:Connect(function()
            ...
            if bt and bt.Character then
                local tp = getBodyPartPos(bt.Character, _G._vanzAimbotBodyPart or "Kepala")
                if tp then
    ...
    _G.vanz.setESP = function(s)
        espEnabled = s
        if not s then
            for p, _ in pairs(espData) do destroyESPTag(p) end
            espData = {}
        end
    end
...
do
    ...
    local function getMuzzlePos()
        ...
        local hrp = getMyHrp()
        if hrp then
            return hrp.Position + Vector3.new(0, 1, 0) + hrp.CFrame.LookVector * 2
        end
        return nil
    end

    local function createTracer()
        if tracerPart then return end
        local part = Instance.new("Part")
        part.Name = "vanzTracer"
        part.Anchored = true
    ...
    local function destroyTracer()
        if tracerPart then
            pcall(function() tracerPart:Destroy() end)
            tracerPart = nil
        end
    end

    local function updateTracer()
        if not silentEnabled or not isTargetValid() then
            if tracerPart and tracerPart.Parent then tracerPart.Transparency = 1 end
            return
        end
        if not tracerPart then createTracer() end
        local from = getMuzzlePos()
        local to = getTargetPos()
        ...
        tracerPart.Size = Vector3.new(0.15, 0.15, dist)
        tracerPart.CFrame = CFrame.new(from, to) * CFrame.new(0, 0, -dist / 2)
    end

    local function installRaycastHook()
        if raycastHookInstalled then return end
        ...
        pcall(function()
            mt.__namecall = newcclosure(function(self, ...)
                if silentEnabled and isTargetValid() then
                    ...
                    local toPos = getTargetPos()
                    if toPos and self == Workspace then
                        if method == "Raycast" then
                            local args = {...}
                            if typeof(args[1]) == "Vector3" and typeof(args[2]) == "Vector3" then
                                local origin = args[1]
                                local dirToTarget = toPos - origin
                                if dirToTarget.Magnitude > 0.5 then
                                    args[2] = dirToTarget.Unit * args[2].Magnitude
                                end
                            end
                        ...
                        elseif method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" then
                            ...
                            for i, arg in ipairs(args) do
                                if typeof(arg) == "Ray" then
                                    local dirToTarget = toPos - arg.Origin
                                    if dirToTarget.Magnitude > 0.5 then
                                        args[i] = Ray.new(arg.Origin, dirToTarget.Unit * arg.Direction.Magnitude)
                                    end
                                end
                            end
                            return oldNamecall(self, table.unpack(args))
    ...
    local function startSilent()
        ...
        silentEnabled = true
        createTracer()
        print("[vanz] silent aim ON, target:", silentTarget and silentTarget.Name or "none")
    end

    local function stopSilent()
        silentEnabled = false
        destroyTracer()
        print("[vanz] silent aim OFF")
    end

    local function refreshList()
        if not listScroll then return end
        for _, row in pairs(targetRows) do
    ...
    Players.PlayerRemoving:Connect(function(plr)
        if silentTarget == plr then silentTarget = nil end
        task.wait(0.1)
        pcall(refreshList)
    end)
    ...
    _G.vanz.silentListScroll = nil
    _G.vanz.setSilentListScroll = function(f) listScroll = f end
...
do
    ...
    -- SILENT AIM
    makeInfo("Silent Aim — kamera gak berubah, tapi arah serangan di-redirect ke target", sb)
    makeToggle("Silent Aim", false, function(s)
        if s then _G.vanz.startSilent() else _G.vanz.stopSilent() end
    end, sb)
    ...
    makeInfo("Player List (auto refresh):", sb)
    ...
    _G.vanz.setSilentListScroll(listScroll)
end
...
print("[vanz] Slim menu + Silent Aim (simple)")
