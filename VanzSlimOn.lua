...
-- ===== AUTO PARRY (TRIPLE LAYER + DOUBLE FIRE) =====
local ATTACK_IDS = {
    "139369275981139",
    "121216847022485",
    "78935059863801",
...
local parryRange = 24
local parryRangeVisualEnabled = false
local parryDebounce = 0.15
local parryDebounceMap = {}
local autoParryLastFire = 0
local autoParryFireCooldown = 0.005
local autoParryRingAdornment = nil
local autoParryRingConn = nil
...
local function extractAnimId(animName)
...
local function isParryAttackAnimation(animName)
    local id = extractAnimId(animName)
    if not id then
        return false
    end
    return ATTACK_SET[id] == true
end
...
local function fireGuiMobDouble()
    fireGuiMob()
    task.spawn(
        function()
            task.wait()
...
local function isKillerInParryRange(char, myHrp)
...
local function tryInstantParry(char)
    ...
    if not isKillerInParryRange(char, myHrp) then
        return
    end
...
local hookedChars = {}
local lastPlayingTrack = {}

local function hookAutoParryChar(char)
    if not char then
        return
    end
    ...
    animator.AnimationPlayed:Connect(
        function(track)
            ...
            if isParryAttackAnimation(aid) then
...
local function watchAutoParryPlayer(plr)
    if plr == LP then
        return
    end
    if plr.Character then
        hookAutoParryChar(plr.Character)
    end
    plr.CharacterAdded:Connect(
        function(char)
            task.wait(0.3)
            hookAutoParryChar(char)
        end
    )
end

for _, plr in ipairs(Players:GetPlayers()) do
    watchAutoParryPlayer(plr)
end
...
local function createAutoParryRing()
    local myHrp = getMyHrp()
    if not myHrp then
        return
    end
    if autoParryRingAdornment and autoParryRingAdornment.Parent == myHrp then
        return
    end
    if autoParryRingAdornment then
        pcall(
            function()
                autoParryRingAdornment:Destroy()
            end
        )
        autoParryRingAdornment = nil
    end
    local ad = Instance.new("CylinderHandleAdornment")
    ad.Name = "vanzAutoParryRing"
    ad.Adornee = myHrp
    ad.AlwaysOnTop = true
    ad.ZIndex = 5
    ad.Height = 0.2
    ...
    ad.Parent = myHrp
    autoParryRingAdornment = ad
end

local function destroyAutoParryRing()
    if autoParryRingAdornment then
        pcall(
            function()
                autoParryRingAdornment:Destroy()
...
local function checkKillerAttackingInParryRange()
    local myHrp = getMyHrp()
    if not myHrp then
        return false
    end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP then
            ...
            if char and char.Parent then
                if isKillerInParryRange(char, myHrp) then
                    local hum = char:FindFirstChildOfClass("Humanoid")
                    if hum then
                        local ok, tracks =
                            pcall(
                        ...
                        if ok and tracks then
                            for _, t in ipairs(tracks) do
                                ...
                                if isParryAttackAnimation(aid) and t.IsPlaying then
                                    return true
                                end
                            end
                        end
...
local function pollKillerAttackAndFire()
    if not autoParryEnabled then
        return
    end
    for _, plr in ipairs(Players:GetPlayers()) do
                    ...
                    if ok and tracks then
                        for _, t in ipairs(tracks) do
                            ...
                            if isParryAttackAnimation(aid) then
                                ...
                                local lp = lastPlayingTrack[char]
                                if lp ~= tid then
                                    lastPlayingTrack[char] = tid
                                    tryInstantParry(char)
                                end
                            end
                        end
...
local function pollKillerTimePositionEarlyFire()
    if not autoParryEnabled or not preFireEnabled then
        return
    end
    local myHrp = getMyHrp()
    if not myHrp then
                            ...
                            for _, t in ipairs(tracks) do
                                ...
                                if isParryAttackAnimation(aid) and t.IsPlaying then
                                    local ok2, tp =
                                        pcall(
                                        function()
                                            return t.TimePosition
                                        end
                                    )
                                    if ok2 and tp and tp < 0.1 then
                                        tryInstantParry(char)
                                    end
                                end
                            end
...
local function pollKillerApproachingPreFire()
    if not autoParryEnabled or not preFireEnabled then
        return
    end
    local myHrp = getMyHrp()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP then
            ...
            if char and char.Parent then
                local killerHrp = getCharHrp(char)
                if killerHrp then
                    local diff = myHrp.Position - killerHrp.Position
                    local dist = diff.Magnitude
                    if dist <= parryRange and dist > 1 then
                        local killerVel = killerHrp.AssemblyLinearVelocity
                        local velMag = killerVel.Magnitude
                        if velMag > 5 then
                            local dot = killerVel.Unit:Dot(diff.Unit)
                            if dot > 0.5 then
                                local hum = char:FindFirstChildOfClass("Humanoid")
                                if hum then
                                    local ok, tracks =
...
local function startAutoParryLoop()
    if autoParryRingConn then
        autoParryRingConn:Disconnect()
    end
    autoParryRingConn =
        RunService.Heartbeat:Connect(
        function()
            if autoParryEnabled then
                pollKillerAttackAndFire()
                pollKillerTimePositionEarlyFire()
                pollKillerApproachingPreFire()
            end
            if parryRangeVisualEnabled then
                if not autoParryRingAdornment or not autoParryRingAdornment.Parent then
                    createAutoParryRing()
                end
                if autoParryRingAdornment then
                    autoParryRingAdornment.Radius = parryRange
                    if checkKillerAttackingInParryRange() then
                        autoParryRingAdornment.Color3 = Color3.fromRGB(0, 255, 80)
                        autoParryRingAdornment.Transparency = 0.15
                    else
                        autoParryRingAdornment.Color3 = Color3.fromRGB(255, 40, 40)
                        autoParryRingAdornment.Transparency = 0.3
                    end
                end
            else
                if autoParryRingAdornment then
                    destroyAutoParryRing()
                end
            end
        end
    )
...
makeInfo("Combat: Anti-Ragdoll + Auto Parry (Triple Layer Pre-Fire)", cb)
makeToggle(
    "Anti Ragdoll",
    false,
    function(state)
...
makeSlider(
    ...
    function(val)
        parryRange = val
        if autoParryRingAdornment then
            autoParryRingAdornment.Radius = val
        end
    end,
    cb
)
...
print("[vanz] Slim menu + Triple Layer Pre-Fire Auto Parry")
