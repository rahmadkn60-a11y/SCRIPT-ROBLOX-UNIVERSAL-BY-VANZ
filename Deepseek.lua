...
local function startJiggle()
    ...
    jiggleAngle = 0;
    jiggleCenter = nil;
    if jiggleConn then jiggleConn:Disconnect(); end;
    jiggleConn = RunService.Heartbeat:Connect(function(dt)
        if not jiggleEnabled then return; end;
        local myChar = LP.Character;
...
local function stopJiggle()
    ...
    jiggleCenter = nil;
    jiggleAngle = 0;
    if jiggleConn then jiggleConn:Disconnect(); jiggleConn = nil; end;
end;
...
makeToggle("Jiggle 360°", false, function(state)
    if state then startJiggle(); else stopJiggle(); end;
end, cb);
