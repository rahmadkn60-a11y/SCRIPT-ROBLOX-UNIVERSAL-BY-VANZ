...
do
    ...
    _G.vanz.ReplicatedStorage = ReplicatedStorage
    _G.vanz.LP = LP
...
-- BLOCK 3: FITUR CORE (wallhack, speed, fly, esp, aimbot, anti-ragdoll, fullbright, server hop, waypoint, teleport)
do
    local Players = _G.vanz.Players
    ...
    local function createESPTag(plr)
        ...
        local line = Instance.new("Frame", _G.vanz.screen)
        ...
        for plr, d in pairs(espData) do
            ...
            if char then
                ...
                if hrp and h then
                    ...
                    end                    local col = _G.vanz.getPlayerColor(plr)
    ...
    local function createAimbotCircle()
        local circle = Instance.new("Frame", _G.vanz.screen)
        ...
        dot.BackgroundColor3 = _G.vanz.CA
    ...
    local function getTargetInCone()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP and plr.Character then
                local skip = false
                if aimbotTeamSafe and _G.vanz.isTeam(plr) then skip = true end
                if not skip then
                    local h = plr.Character:FindFirstChildOfClass("Humanoid")
                    if h and h.Health > 0 then
    ...
    _G.vanz.getWallhack = function() return wallhackEnabled end
    ...
    _G.vanz.setESP = function(s) espEnabled = s if s then refreshESP() else clearAllESP() end end
    _G.vanz.startAimbot = startAimbot
    _G.vanz.stopAimbot = stopAimbot
    _G.vanz.updateAimbotCircleVisual = updateAimbotCircleVisual
    _G.vanz.setAimbotPOV = function(v) aimbotPOV = v updateAimbotCircleVisual() end
    ...
    _G.vanz.markWaypoint = markWaypoint
    _G.vanz.clearWaypoint = clearWaypoint
    _G.vanz.teleportToWaypoint = teleportToWaypoint
    ...
    _G.vanz.setFollowEnabled = function(s)
        followEnabled = s
        if s then if teleportSelected then startFollow() else followEnabled = false end else stopFollow() end
    end
    _G.vanz.findBestEspPart = findBestEspPart
    _G.vanz.getBodyPartPos = getBodyPartPos
end

-- BLOCK 4: AUTO PARRY (radius based)
do
    local Players = _G.vanz.Players
    local RunService = _G.vanz.RunService
    local Workspace = _G.vanz.Workspace
    ...
    local parryRange = 20
    local parryRingEnabled = true
    local parryDebounce = 0.1
    local parryDebounceMap = {}
    local autoParryLastFire = 0
    local autoParryFireCooldown = 0.005
    local parryRingOuter = nil
    local parryRingConn = nil
    ...
    task.spawn(function()
        while true do
            task.wait(2)
            if not cachedGuiMob or not cachedGuiMob.Parent then
                cachedGuiMob = getGuiMob()
            end
        end
    end)
    ...
    local function fireGuiMob()
        ...
        if not guiMob then return false end
        pcall(function()
    ...
    local function isKillerInRange(char, myHrp)
        if not char or not myHrp then return false end
        local killerHrp = getCharHrp(char)
        if not killerHrp then return false end
        return (killerHrp.Position - myHrp.Position).Magnitude <= parryRange
    end

    local function hasAnyKillerInRange()
        local myHrp = getMyHrp()
        if not myHrp then return false end
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP then
                local char = getKillerChar(plr)
                if char and isKillerInRange(char, myHrp) then return true end
            end
        end
        return false
    end

    local function tryInstantParry(char)
        if not autoParryEnabled then return end
        local now = tick()
        if now - autoParryLastFire < autoParryFireCooldown then return end
        local myHrp = getMyHrp()
        if not myHrp then return end
        if not isKillerInRange(char, myHrp) then return end
    ...
    local function createParryRing()
        if parryRingOuter then
            for _, p in ipairs(parryRingOuter) do pcall(function() p:Destroy() end) end
        end
        parryRingOuter = {}
        if not getMyHrp() then return end
        for i = 1, RING_SEGMENTS do
            local outer = Instance.new("Part")
            outer.Name = "vanzRingOuter"
            outer.Shape = Enum.PartType.Block
            outer.Size = Vector3.new(0.15, 0.05, 0.5)
            outer.Anchored = true
            outer.CanCollide = false
            outer.CanQuery = false
            outer.CanTouch = false
            outer.Material = Enum.Material.Neon
            outer.Color = Color3.fromRGB(255, 40, 40)
            outer.Transparency = 0.3
            outer.CastShadow = false
            outer.Parent = Workspace
            parryRingOuter[i] = outer
        end
    end

    local function destroyParryRing()
        if parryRingOuter then
            for _, p in ipairs(parryRingOuter) do pcall(function() p:Destroy() end) end
            parryRingOuter = nil
        end
    ...
    local function updateParryRing()
        if not parryRingEnabled then return end
        local myHrp = getMyHrp()
        if not myHrp then
            if parryRingOuter then destroyParryRing() end
            return
        end
        if not parryRingOuter then createParryRing() end
        if not parryRingOuter then return end
        local color = hasAnyKillerInRange() and Color3.fromRGB(0, 255, 80) or Color3.fromRGB(255, 40, 40)
        local basePos = myHrp.Position
        for i = 1, RING_SEGMENTS do
            local angle = (i / RING_SEGMENTS) * math.pi * 2
            local dir = Vector3.new(math.cos(angle), 0, math.sin(angle))
            local pos = basePos + dir * parryRange
            local outer = parryRingOuter[i]
            if outer and outer.Parent then
                outer.CFrame = CFrame.new(pos, pos + dir)
                outer.Color = color
            end
        end
    end

    local function pollKillerAndParry()
        if not autoParryEnabled then return end
        local myHrp = getMyHrp()
        if not myHrp then return end
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP then
                local char = getKillerChar(plr)
                if char and isKillerInRange(char, myHrp) then
                    tryInstantParry(char)
                end
            end
        end
    end
    ...
    local parryConn = RunService.Heartbeat:Connect(function()
        if autoParryEnabled then pollKillerAndParry() end
        updateParryRing()
    end)

    LP.CharacterAdded:Connect(function()
        task.wait(0.5)
        if parryRingEnabled then
            destroyParryRing()
            createParryRing()
        end
    end)
    ...
    _G.vanz.setParryRingEnabled = function(s)
        parryRingEnabled = s
        if not s then destroyParryRing() else createParryRing() end
    end
end

-- BLOCK 5: UNIFIED SILENT AIM (6 MEKANISME)
do
    local Players = _G.vanz.Players
    local RunService = _G.vanz.RunService
    local Workspace = _G.vanz.Workspace
    local ReplicatedStorage = _G.vanz.ReplicatedStorage
    local LP = _G.vanz.LP
    local T = _G.vanz.T
    ...
    local silentRemoteHooked = false
    local silentHitboxEnabled = true
    local silentRaycastEnabled = true
    local silentMouseEnabled = true
    local silentCameraRayEnabled = true
    local silentRemoteEnabled = true
    local silentFindPartOnRayEnabled = true
    local silentHitboxData = {}
    local silentOriginalHit = nil
    local silentMouse = nil
    local silentHookFunctions = {}

    pcall(function() silentMouse = LP:GetMouse() end)

    local function getBodyPartPos()
        if not silentTarget then return nil end
        local char = silentTarget.Character
        if not char then return nil end
        if silentBodyPart == "Kepala" then
            local hd = char:FindFirstChild("Head")
            if hd then return hd.Position end
        elseif silentBodyPart == "Leher" then
            local hd = char:FindFirstChild("Head")
            if hd then return hd.Position - Vector3.new(0, 0.5, 0) end
        elseif silentBodyPart == "Badan" then
            local ut = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
            if ut then return ut.Position end
        elseif silentBodyPart == "Paha" then
            local ul = char:FindFirstChild("LeftUpperLeg") or char:FindFirstChild("RightUpperLeg")
            if ul then return ul.Position end
            local lt = char:FindFirstChild("LowerTorso")
            if lt then return lt.Position - Vector3.new(0, 1, 0) end
        elseif silentBodyPart == "Kaki" then
            local ft = char:FindFirstChild("LeftFoot") or char:FindFirstChild("RightFoot")
    ...
    local function getBodyPart()
        ...
        elseif silentBodyPart == "Paha" then
            return char:FindFirstChild("LeftUpperLeg") or char:FindFirstChild("RightUpperLeg") or char:FindFirstChild("LowerTorso")
        elseif silentBodyPart == "Kaki" then
            return char:FindFirstChild("LeftFoot") or char:FindFirstChild("RightFoot") or char:FindFirstChild("LeftLowerLeg") or char:FindFirstChild("RightLowerLeg")
        end
        return char:FindFirstChild("HumanoidRootPart")
    end

    local function isTargetValid()
        if not silentTarget then return false end
        local char = silentTarget.Character
    ...
    local function applyHitbox()
        if not silentHitboxEnabled then return end
        if not isTargetValid() then return end
        local char = silentTarget.Character
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if not hrp then return end
        if silentHitboxData[char] then return end
        silentHitboxData[char] = {hrp = hrp, size = hrp.Size, transparency = hrp.Transparency, canCollide = hrp.CanCollide}
    ...
    local function removeHitbox()
        for char, d in pairs(silentHitboxData) do
            if d.hrp and d.hrp.Parent then
                pcall(function()
                    d.hrp.Size = d.size
                    d.hrp.Transparency = d.transparency
                    d.hrp.CanCollide = d.canCollide
                end)
            end
        end
        silentHitboxData = {}
    ...
    -- MEKANISME 2,3,4,6: HOOK via metatable
    local function setupHooks()
        if silentHooked then return end
        silentHooked = true
        local mt = nil
        pcall(function()
            mt = getrawmetatable and getrawmetatable(game)
        end)
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            local args = {...}
            if silentEnabled and isTargetValid() then
                local targetPart = getBodyPart()
                local targetPos = getBodyPartPos()
                if targetPart and targetPos then
                    if method == "FireServer" or method == "InvokeServer" then
                        if silentRemoteEnabled then
                    ...
                    elseif self == Workspace and method == "Raycast" then
                        if silentRaycastEnabled then
                            ...
                            if typeof(origin) == "Vector3" and typeof(direction) == "Vector3" then
                                ...
                                if dist > 0.1 then
                                    args[2] = dirToTarget.Unit * direction.Magnitude
                                end
                            end
                        end
                    elseif self == Workspace and (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist") then
                        if silentFindPartOnRayEnabled then
                            for i, arg in ipairs(args) do
                                if typeof(arg) == "Ray" then
                                    local rayOrigin = arg.Origin
                                    local dirToTarget = targetPos - rayOrigin
                                    args[i] = Ray.new(rayOrigin, dirToTarget.Unit * arg.Direction.Magnitude)
                                end
                            end
                        end
                    end
    ...
    -- MEKANISME 5: CAMERA RAY REDIRECT
    local function hookCameraRay()
        pcall(function()
            local cam = Workspace.CurrentCamera
            if not cam then return end
            local mt = getrawmetatable and getrawmetatable(cam)
            if not mt then return end
            local oldNamecall = mt.__namecall
            if not oldNamecall then return end
            setreadonly(mt, false)
            mt.__namecall = newcclosure(function(self, ...)
                local method = getnamecallmethod and getnamecallmethod() or ""
                if silentEnabled and silentCameraRayEnabled and isTargetValid() then
                    if method == "ViewportPointToRay" or method == "ScreenPointToRay" then
                        ...
                        if targetPos then
                            ...
                            local dirToTarget = targetPos - origin
                            return Ray.new(origin, dirToTarget.Unit)
                        end
                    end
                end
    ...
    -- MEKANISME 6: MOUSE HIT SPOOF
    local function hookMouse()
        if not silentMouse then return end
        silentMouse.Button1Down:Connect(function()
            if not silentEnabled or not silentMouseEnabled then return end
            if not isTargetValid() then return end
            local targetPos = getBodyPartPos()
            if targetPos then
                silentOriginalHit = silentMouse.Hit
                pcall(function()
                    silentMouse.Hit = CFrame.new(targetPos)
                    silentMouse.Target = getBodyPart()
                end)
            end
        end)
        silentMouse.Button1Up:Connect(function()
            if silentOriginalHit and silentMouse then
                pcall(function()
                    silentMouse.Hit = silentOriginalHit
                end)
            end
        end)
    end
    ...
    local function startSilent()
        if silentEnabled then return end
        silentEnabled = true
        setupHooks()
        hookCameraRay()
        if silentHitboxEnabled then applyHitbox() end
        if not silentConn then
            silentConn = RunService.Heartbeat:Connect(function()
                if silentEnabled and silentHitboxEnabled then
                    applyHitbox()
                end
            end)
        end
    ...
    local silentTargetRows = {}
    local silentListFrame = nil

    local function refreshSilentList()
        if not silentListFrame then return end
        for _, row in pairs(silentTargetRows) do
            pcall(function() row:Destroy() end)
        end
        silentTargetRows = {}
        if silentTarget and silentTarget.Parent == nil then silentTarget = nil end
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP then
                local row = Instance.new("TextButton")
                row.Size = UDim2.new(1, 0, 0, 34)
                ...
                row.MouseButton1Click:Connect(function()
                    silentTarget = plr
                    for p, r in pairs(silentTargetRows) do
                        if p == plr then
                            r.BackgroundColor3 = T.sel
                            r.TextColor3 = Color3.fromRGB(255, 255, 255)
                        else
    ...
    _G.vanz.startSilent = startSilent
    _G.vanz.stopSilent = stopSilent
    _G.vanz.setSilentTarget = function(p) silentTarget = p end
    _G.vanz.setSilentBodyPart = function(p) silentBodyPart = p end
    _G.vanz.setSilentListFrame = function(f) silentListFrame = f end
    _G.vanz.refreshSilentList = refreshSilentList
    _G.vanz.setSilentMekanisme = function(key, val)
        if key == "hitbox" then silentHitboxEnabled = val
        elseif key == "raycast" then silentRaycastEnabled = val
        elseif key == "mouse" then silentMouseEnabled = val
        elseif key == "cameraRay" then silentCameraRayEnabled = val
        elseif key == "remote" then silentRemoteEnabled = val
        elseif key == "findPartOnRay" then silentFindPartOnRayEnabled = val end
    end
...
do
    ...
    makeToggle("Anti Ragdoll", false, function(state)
        if state then _G.vanz.startAntiRagdoll() else _G.vanz.stopAntiRagdoll() end
    end, cb)
    makeToggle("Auto Parry", false, function(state)
        _G.vanz.setAutoParry(state)
    end, cb)
    makeNum("Parry Range", 20, function(val)
        _G.vanz.setParryRange(val)
    end, cb)
    makeToggle("Ring Visual", true, function(state)
        _G.vanz.setParryRingEnabled(state)
    end, cb)

    -- VISUAL
    makeInfo("Visual: ESP + Fullbright", vb)
    makeToggle("ESP", false, function(state)
        _G.vanz.setESP(state)
    end, vb)
    ...
    makeSlider("Brightness", 0, 10, 3, function(val)
        _G.vanz.setFullbrightLevel(val)
    end, vb)

    -- MOVEMENT
    makeInfo("Movement: Wallhack, Speed, Jump, Fly", mb)
    makeToggle("Wallhack", false, function(state)
        _G.vanz.setWallhack(state)
    end, mb)
    makeToggle("Speed Hack", false, function(state)
        _G.vanz.setSpeed(state)
    end, mb)
    makeNum("Speed", 100, function(num)
        _G.vanz.setCurrentSpeed(num)
    end, mb)
    local infJump = false
    ...
    makeToggle("Fly", false, function(state)
        if state then _G.vanz.startFly() else _G.vanz.stopFly() end
    end, mb)

    -- UTILITY
    makeInfo("Utility: Server Hop", ub)
    local hopInfo = Instance.new("TextLabel", ub)
    hopInfo.Size = UDim2.new(1, 0, 0, 44)
    hopInfo.BackgroundColor3 = T.row
    hopInfo.Text = "Server Hop: Ramai = player terbanyak. Sepi = player paling sedikit."
    ...
    makeToggle("Follow", false, function(state)
        _G.vanz.setFollowEnabled(state)
    end, tpb)

    makeInfo("Waypoint: tandai posisi, teleport balik kapan aja", tpb)
    makeToggle("Waypoint (Mark)", false, function(state)
        if state then _G.vanz.markWaypoint() else _G.vanz.clearWaypoint() end
    end, tpb)
    makeButton("TELEPORT KE WAYPOINT", function()
        _G.vanz.teleportToWaypoint()
    end, tpb)

    -- AIMBOT
    makeInfo("Aimbot: lock ke body part", ab)
    makeToggle("Aimbot", false, function(state)
        if state then _G.vanz.startAimbot() else _G.vanz.stopAimbot() end
    end, ab)
    makeToggle("Team Safe (Aimbot)", true, function(state)
        _G.vanz.setAimbotTeamSafe(state)
    end, ab)
    makeSlider("POV", 5, 360, 60, function(val)
        _G.vanz.setAimbotPOV(val)
    end, ab)
    ...
    for i, label in ipairs(bpOptions) do
        ...
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
        bpButtons[i] = btn
        btn.MouseButton1Click:Connect(function() setBPSelected(i) end)
    end
    _G._vanzAimbotBodyPart = "Kepala"

    -- SILENT AIM
    makeInfo("Silent Aim: aktifin 6 mekanisme sekaligus. Pilih target, tembakan auto ke dia.", sb)
    makeToggle("Silent Aim", false, function(state)
        if state then _G.vanz.startSilent() else _G.vanz.stopSilent() end
    end, sb)

    local bpSelectorSilent = Instance.new("Frame", sb)
    bpSelectorSilent.Size = UDim2.new(1, 0, 0, 70)
    bpSelectorSilent.BackgroundColor3 = T.row
    bpSelectorSilent.BorderSizePixel = 0
    Instance.new("UICorner", bpSelectorSilent).CornerRadius = UDim.new(0, 8)
    local bptitleSilent = Instance.new("TextLabel", bpSelectorSilent)
    bptitleSilent.Size = UDim2.new(1, -20, 0, 20)
    bptitleSilent.Position = UDim2.new(0, 10, 0, 4)
    bptitleSilent.BackgroundTransparency = 1
    bptitleSilent.Text = "Hit Target:"
    bptitleSilent.TextColor3 = T.dim
    bptitleSilent.TextSize = 11
    bptitleSilent.Font = Enum.Font.Gotham
    bptitleSilent.TextXAlignment = Enum.TextXAlignment.Left
    local bpRowSilent = Instance.new("Frame", bpSelectorSilent)
    bpRowSilent.Size = UDim2.new(1, -16, 0, 34)
    bpRowSilent.Position = UDim2.new(0, 8, 0, 28)
    bpRowSilent.BackgroundTransparency = 1
    local bpLayoutSilent = Instance.new("UIListLayout", bpRowSilent)
    bpLayoutSilent.FillDirection = Enum.FillDirection.Horizontal
    bpLayoutSilent.Padding = UDim.new(0, 4)
    local bpOptionsSilent = {"Kepala", "Leher", "Badan", "Paha", "Kaki"}
    local bpButtonsSilent = {}
    local function setBPSilent(idx)
        for i, b in ipairs(bpButtonsSilent) do
            if i == idx then
                b.BackgroundColor3 = T.accent
                b.TextColor3 = Color3.fromRGB(255, 255, 255)
            else
        ...
        _G.vanz.setSilentBodyPart(bpOptionsSilent[idx])
    end
    for i, label in ipairs(bpOptionsSilent) do
        local btn = Instance.new("TextButton", bpRowSilent)
        btn.Size = UDim2.new(0, 46, 0, 30)
        btn.BackgroundColor3 = (i == 1) and T.accent or T.off
        btn.Text = label
        btn.TextColor3 = (i == 1) and Color3.fromRGB(255, 255, 255) or T.text
        ...
        btn.LayoutOrder = i
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
        bpButtonsSilent[i] = btn
        btn.MouseButton1Click:Connect(function() setBPSilent(i) end)
    end

    makeInfo("Player List (pilih target):", sb)

    local silentFrame = Instance.new("Frame", sb)
    silentFrame.Size = UDim2.new(1, 0, 0, 180)
    silentFrame.BackgroundColor3 = T.bg
    silentFrame.BorderSizePixel = 0
    silentFrame.ClipsDescendants = true
    Instance.new("UICorner", silentFrame).CornerRadius = UDim.new(0, 8)

    local silentScroll = Instance.new("ScrollingFrame", silentFrame)
    silentScroll.Size = UDim2.new(1, -8, 1, -8)
    silentScroll.Position = UDim2.new(0, 4, 0, 4)
    silentScroll.BackgroundTransparency = 1
    silentScroll.BorderSizePixel = 0
    silentScroll.ScrollBarThickness = 4
    silentScroll.ScrollBarImageColor3 = T.accent
    silentScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
    silentScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
    local silentLayout = Instance.new("UIListLayout", silentScroll)
    silentLayout.Padding = UDim.new(0, 4)
    silentLayout.SortOrder = Enum.SortOrder.LayoutOrder
    _G.vanz.setSilentListFrame(silentScroll)

    makeButton("REFRESH TARGET LIST", function()
        _G.vanz.refreshSilentList()
    end, sb)

    makeInfo("Mekanisme Aktif: Hitbox Expand, Raycast Redirect, FindPartOnRay Redirect, Mouse.Hit Spoof, Camera Ray Redirect, Remote Arg Inject", sb)
end
...
print("[vanz] Slim menu + Auto Parry + Unified Silent Aim (6 mekanisme)")
