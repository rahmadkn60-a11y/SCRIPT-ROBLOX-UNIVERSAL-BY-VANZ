...
-- ===== JIGGLE 360 =====
local jiggleEnabled = false
local jiggleTriggerDist = 6
local jiggleOrbitRadius = 5
local jiggleConn = nil
local jiggleAngle = 0
local jiggleCenter = nil
local jiggleActive = false

local function hasEnemyInRange()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP and plr.Character then
            if not isTeam(plr) then
                local h = plr.Character:FindFirstChildOfClass("Humanoid")
                if h and h.Health > 0 then
                    local hrp = plr.Character:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        local dist = (hrp.Position - myHrp.Position).Magnitude
                        if dist <= jiggleTriggerDist then
                            return true
                        end
                    end
                end
...
local function startJiggle()
    ...
    jiggleCenter = nil
    if jiggleConn then
        jiggleConn:Disconnect()
    end
    jiggleConn =
        RunService.Heartbeat:Connect(
        function(dt)
            if not jiggleEnabled then
                return
            end
            local myChar = LP.Character
            if not myChar then
            ...
            local threat = hasEnemyInRange()
            if threat then
                if not jiggleActive then
                    jiggleActive = true
                    jiggleCenter = myHrp.Position
                    jiggleAngle = 0
                end
                local sp = myHum.WalkSpeed
                if sp <= 0 then
                    sp = 16
                end
                local angSpeed = sp / jiggleOrbitRadius
                jiggleAngle = jiggleAngle + angSpeed * dt
                if jiggleAngle > math.pi * 2 then
                    jiggleAngle = jiggleAngle - math.pi * 2
                end
                local cx = jiggleCenter.X + math.cos(jiggleAngle) * jiggleOrbitRadius
                local cz = jiggleCenter.Z + math.sin(jiggleAngle) * jiggleOrbitRadius
                local cy = jiggleCenter.Y
                local pr = RaycastParams.new()
                pr.FilterType = Enum.RaycastFilterType.Exclude
                pr.FilterDescendantsInstances = {myChar}
                pr.IgnoreWater = true
                local groundRes = Workspace:Raycast(Vector3.new(cx, cy + 5, cz), Vector3.new(0, -200, 0), pr)
                if groundRes and groundRes.Instance then
                    cy = groundRes.Position.Y + 3
                end
                local dirX = -math.sin(jiggleAngle)
                local dirZ = math.cos(jiggleAngle)
                local lookDir = Vector3.new(dirX, 0, dirZ)
                if lookDir.Magnitude > 0.01 then
                    myHrp.CFrame = CFrame.new(Vector3.new(cx, cy, cz), Vector3.new(cx + lookDir.X, cy, cz + lookDir.Z))
                else
                    myHrp.CFrame = CFrame.new(cx, cy, cz)
                end
...
makeInfo("Combat: Anti-Ragdoll + Jiggle 360°", cb)
makeToggle(
    "Anti Ragdoll",
    false,
    function(state)
...
print("[vanz] Slim menu + Jiggle 360°")
