...
local function getAnalogMove()
    ...
    if ff.Magnitude < 0.01 then
        ff = Vector3.new(0, 0, -1)
    end
    ff = ff.Unit
    local fr = Vector3.new(rt.X, 0, rt.Z)
    if fr.Magnitude < 0.01 then
        fr = ff:Cross(Vector3.new(0, 1, 0))
    end
    fr = fr.Unit
    local wm = (ff * -mv.Z) + (fr * mv.X)
    if wm.Magnitude > 0.01 then
        return wm.Unit
    end
    return Vector3.zero
end
...
local function startFly()
    ...
    flyConn =
        ...
        function(dt)
            ...
            if not flyAlign or not flyAlign.Parent then
                return
            end
...
local function getBodyPartPos(char, partName)
    ...
    elseif partName == "Leher" then
        local hd = char:FindFirstChild("Head")
        if hd and hd:IsA("BasePart") then
            return hd.Position - Vector3.new(0, 0.5, 0)
        end
    elseif partName == "Badan" then
        local ut = char:FindFirstChild("UpperTorso")
        if ut and ut:IsA("BasePart") then
            return ut.Position
        end
    ...
    elseif partName == "Kaki" then
        ...
        if ll and ll:IsA("BasePart") then
            return ll.Position
        end
...
local function getTargetInCone()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP and plr.Character then
            local skip = false
            if aimbotTeamSafe and isTeam(plr) then
                skip = true
            end
            if not skip then
                local h = plr.Character:FindFirstChildOfClass("Humanoid")
...
local function startAimbot()
    aimbotEnabled = true
    if aimbotCircleFrame then
        aimbotCircleFrame.Visible = true
    end
...
local function stopAimbot()
    ...
    if aimbotCircleFrame then
        aimbotCircleFrame.Visible = false
    end
    aimbotTarget = nil
    aimbotLocked = false
    aimbotOutOfConeTimer = 0
...
local function startAntiRagdoll()
    ...
    antiRagdollConn =
        ...
        function()
            ...
            local s = h:GetState()
            if
                s == Enum.HumanoidStateType.FallingDown or s == Enum.HumanoidStateType.Ragdoll or
...
local function hopToServer(mode)
    ...
    local url =
        ...
        PlaceId .. "/servers/Public?sortOrder=Asc&limit=100&excludeFullGames=true"
    local ok, response =
        pcall(
        function()
...
local function destroyWaypointMarker()
    if waypointMarkerPart then
        pcall(
            function()
                waypointMarkerPart:Destroy()
            end
        )
        waypointMarkerPart = nil
    end
    if waypointMarkerHL then
        pcall(
            function()
                waypointMarkerHL:Destroy()
            end
        )
        waypointMarkerHL = nil
    end
    if waypointMarkerBox then
        pcall(
            function()
                waypointMarkerBox:Destroy()
            end
        )
        waypointMarkerBox = nil
    end
    if waypointMarkerSphere then
        pcall(
            function()
                waypointMarkerSphere:Destroy()
            end
        )
        waypointMarkerSphere = nil
    end
    if waypointMarkerBB then
        pcall(
            function()
                waypointMarkerBB:Destroy()
            end
        )
        waypointMarkerBB = nil
    end
    if waypointMarkerLine then
        pcall(
            function()
                waypointMarkerLine:Destroy()
            end
        )
        waypointMarkerLine = nil
    end
end
...
local function destroyOriginMarker()
    if teleportV2MarkerPart then
        pcall(
            function()
                teleportV2MarkerPart:Destroy()
            end
        )
    end
    if teleportV2MarkerHL then
        pcall(
            function()
                teleportV2MarkerHL:Destroy()
            end
        )
    end
    if teleportV2MarkerBox then
        pcall(
            function()
                teleportV2MarkerBox:Destroy()
            end
        )
    end
    if teleportV2MarkerSphere then
        pcall(
            function()
                teleportV2MarkerSphere:Destroy()
            end
        )
    end
    if teleportV2MarkerBB then
        pcall(
            function()
                teleportV2MarkerBB:Destroy()
            end
        )
    end
...
local function stopTeleportV2()
    if not teleportV2Enabled then
        return
    end
    teleportV2Enabled = false
    if teleportV2Conn then
        teleportV2Conn:Disconnect()
        teleportV2Conn = nil
    end
    local char = LP.Character
    if char then
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp then
            if teleportV2PhantomPos then
                hrp.CFrame = CFrame.new(teleportV2PhantomPos)
            end
            hrp.Anchored = false
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.AssemblyAngularVelocity = Vector3.zero
        ...
        if hum then
            ...
            if teleportV2SavedWalkSpeed then
                hum.WalkSpeed = teleportV2SavedWalkSpeed
            else
                hum.WalkSpeed = 16
            end
            if teleportV2SavedJumpPower then
                hum.JumpPower = teleportV2SavedJumpPower
            end
            pcall(
                function()
...
local function startTeleportV2()
    ...
    teleportV2Conn =
        ...
        function(dt)
            ...
            if checkCollision(c, fromPos, toPos) then
                return
            end
...
local function refreshGuiMobCache()
...
local function hasAnyKillerInRange()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP then
            ...
            if char then
                if isKillerInRange(char, myHrp) then
                    return true
                end
            end
        end
...
local function tryInstantParry(char)
    ...
    local now = tick()
    if now - autoParryLastFire < autoParryFireCooldown then
        return
    end
    local myHrp = getMyHrp()
    if not myHrp then
    ...
    if not isKillerInRange(char, myHrp) then
        return
    end
    local key = tostring(char:GetDebugId())
    local last = parryDebounceMap[key] or 0
    if now - last < parryDebounce then
        return
    end
...
local function createParryRing()
    ...
    for i = 1, RING_SEGMENTS do
        ...
        outer.Size = Vector3.new(0.15, 0.05, 0.4)
        outer.Anchored = true
...
local function destroyParryRing()
    ...
    if parryRingInner then
        for _, p in ipairs(parryRingInner) do
            pcall(
                function()
                    p:Destroy()
                end
...
local function updateParryRing()
    ...
    if not parryRingOuter or not parryRingInner then
        createParryRing()
        if not parryRingOuter then
            return
        end
    end
    ...
    for i = 1, RING_SEGMENTS do
        ...
        local lookCF = CFrame.new(pos, pos + dir)
        local outer = parryRingOuter[i]
        local inner = parryRingInner[i]
        if outer and outer.Parent then
            outer.CFrame = lookCF
            outer.Color = color
        end
        if inner and inner.Parent then
            local innerPos = basePos + dir * (parryRange - 0.3)
            inner.CFrame = CFrame.new(innerPos, innerPos + dir)
            inner.Color = color
        end
    end
end
...
local function startAutoParryLoop()
    if parryRingConn then
        parryRingConn:Disconnect()
    end
    parryRingConn =
        RunService.Heartbeat:Connect(
        function()
...
local silentAimMode = "camera"
local silentAimConn = nil
local silentAimRemoteHooks = {}
local silentAimMouse = nil
local silentAimOriginalHit = nil
local silentAimLastCFrame = nil

pcall(
    function()
        silentAimMouse = LP:GetMouse()
    end
)
...
local function getSilentTargetPos()
    ...
    local hrp =
        char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or
        char:FindFirstChild("Head")
    if not hrp then
        return nil
    end
...
local function isSilentTargetValid()
    if not silentAimTarget then
        return false
    end
    local char = silentAimTarget.Character
...
local function isTargetInRange()
    if not isSilentTargetValid() then
        return false
    end
    local myHrp = getMyHrp()
    if not myHrp then
    ...
    local tp = getSilentTargetPos()
    if not tp then
        return false
    end
    return (tp - myHrp.Position).Magnitude <= 500
end

local function updateSilentCamera()
    local cam = Workspace.CurrentCamera
    if not cam then
        return
    end
    if not silentAimEnabled then
        if silentAimLastCFrame then
            cam.CFrame = silentAimLastCFrame
            silentAimLastCFrame = nil
        end
        return
    end
    ...
    if silentAimMode == "camera" or silentAimMode == "hybrid" then
        local tp = getSilentTargetPos()
        if tp then
            silentAimLastCFrame = cam.CFrame
            cam.CFrame = CFrame.new(cam.CFrame.Position, tp)
        end
    end
end

local function spoofMouse()
    if not silentAimEnabled then
        return
    end
    if not silentAimMouse then
        return
    end
    if not isTargetInRange() then
        return
    end
    if silentAimMode == "mouse" or silentAimMode == "hybrid" then
        local tp = getSilentTargetPos()
        if tp then
            silentAimOriginalHit = silentAimMouse.Hit
            pcall(
                function()
                    silentAimMouse.Hit = CFrame.new(tp)
                    silentAimMouse.Target = getSilentTargetPart()
                end
            )
            task.delay(
                0.05,
                function()
                    if silentAimOriginalHit and silentAimMouse then
                        pcall(
                            function()
                                silentAimMouse.Hit = silentAimOriginalHit
                            end
                        )
                    end
                end
...
local function hookGunRemote(remote)
    if not remote or silentAimRemoteHooks[remote] then
        return
    end
    silentAimRemoteHooks[remote] = true
    ...
    mt.__namecall =
        ...
        function(self, ...)
            ...
            if self == remote and (method == "FireServer" or method == "InvokeServer") then
                if silentAimEnabled and isTargetInRange() then
                    local tp = getSilentTargetPos()
                    if tp then
                        local args = {...}
                        for i, arg in ipairs(args) do
                            if typeof(arg) == "CFrame" then
                                local dist = (arg.Position - tp).Magnitude
                                if dist < 500 then
                                    args[i] = CFrame.new(arg.Position, tp)
                                end
                            elseif typeof(arg) == "Vector3" then
                                local dist = (arg - tp).Magnitude
                                if dist < 500 then
                                    args[i] = tp
                                end
                            elseif typeof(arg) == "Instance" then
                                if arg:IsA("BasePart") then
                                    args[i] = getSilentTargetPart() or arg
                                end
                            end
                        end
...
local function scanForGunRemotes()
    ...
    for _, obj in ipairs(remotesFolder:GetDescendants()) do
        if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
            ...
            if
                n:find("fire") or n:find("shot") or n:find("shoot") or n:find("hit") or n:find("attack") or
                    n:find("bullet") or
                    n:find("damage")
             then
                hookGunRemote(obj)
...
local function hookMouseButton()
    if not silentAimMouse then
        return
    end
    silentAimMouse.Button1Down:Connect(
        function()
    ...
    silentAimMouse.Button1Up:Connect(
        function()
            if silentAimOriginalHit and silentAimMouse then
                pcall(
                    function()
...
local function startSilentAim()
    ...
    if silentAimMode == "remote" or silentAimMode == "hybrid" then
        scanForGunRemotes()
    end
    print("[silent] enabled mode=" .. silentAimMode)
end
...
local function refreshSilentTargetList()
    if not silentListFrame then
        return
    end
    ...
    silentTargetRows = {}
    if silentAimTarget and silentAimTarget.Parent == nil then
        silentAimTarget = nil
    end
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LP then
...
Players.PlayerAdded:Connect(
    function()
        task.wait(0.5)
        refreshSilentTargetList()
    end
)

Players.PlayerRemoving:Connect(
    function(plr)
        if silentAimTarget == plr then
            silentAimTarget = nil
        end
        task.wait(0.1)
        refreshSilentTargetList()
...
local sb = tabContainers["silent"]
makeInfo("Silent Aim: pilih target, tembakan otomatis ke dia", sb)
makeToggle(
    "Silent Aim",
    false,
    function(state)
...
local modeSelector = Instance.new("Frame", sb)
modeSelector.Size = UDim2.new(1, 0, 0, 90)
modeSelector.BackgroundColor3 = T.row
modeSelector.BorderSizePixel = 0
Instance.new("UICorner", modeSelector).CornerRadius = UDim.new(0, 8)
local modeTitle = Instance.new("TextLabel", modeSelector)
modeTitle.Size = UDim2.new(1, -20, 0, 20)
modeTitle.Position = UDim2.new(0, 10, 0, 4)
modeTitle.BackgroundTransparency = 1
modeTitle.Text = "Mode:"
modeTitle.TextColor3 = T.dim
modeTitle.TextSize = 11
modeTitle.Font = Enum.Font.Gotham
modeTitle.TextXAlignment = Enum.TextXAlignment.Left
local modeRow1 = Instance.new("Frame", modeSelector)
modeRow1.Size = UDim2.new(1, -16, 0, 26)
modeRow1.Position = UDim2.new(0, 8, 0, 26)
modeRow1.BackgroundTransparency = 1
local modeRow1Lay = Instance.new("UIListLayout", modeRow1)
modeRow1Lay.FillDirection = Enum.FillDirection.Horizontal
modeRow1Lay.Padding = UDim.new(0, 4)
local modeRow2 = Instance.new("Frame", modeSelector)
modeRow2.Size = UDim2.new(1, -16, 0, 26)
modeRow2.Position = UDim2.new(0, 8, 0, 56)
modeRow2.BackgroundTransparency = 1
local modeRow2Lay = Instance.new("UIListLayout", modeRow2)
modeRow2Lay.FillDirection = Enum.FillDirection.Horizontal
modeRow2Lay.Padding = UDim.new(0, 4)
...
local modeButtons = {}
local function setSilentMode(idx)
    silentAimMode = modeOptions[idx][1]
    ...
    print("[silent] mode = " .. silentAimMode)
end
for i, opt in ipairs(modeOptions) do
    local parent = (i <= 2) and modeRow1 or modeRow2
...
makeInfo(
    "Camera: override kamera. Mouse: override Mouse.Hit. Remote: hook remote senjata. Hybrid: semua sekaligus.",
    sb
)

local silentLabel = Instance.new("TextLabel", sb)
silentLabel.Size = UDim2.new(1, 0, 0, 20)
silentLabel.BackgroundTransparency = 1
silentLabel.Text = "Player List (pilih target):"
silentLabel.TextColor3 = T.dim
...
makeInfo("Combat: Anti-Ragdoll + Auto Parry (Radius Based)", cb)
makeToggle(
    "Anti Ragdoll",
    false,
    function(state)
...
makeToggle(
    ...
    function(state)
        if state then
            ...
            if fullbrightSaved.Brightness then
                Lighting.Brightness = fullbrightSaved.Brightness
            end
            if fullbrightSaved.Ambient then
                Lighting.Ambient = fullbrightSaved.Ambient
            end
            if fullbrightSaved.OutdoorAmbient then
                Lighting.OutdoorAmbient = fullbrightSaved.OutdoorAmbient
            end
            if fullbrightSaved.ClockTime then
                Lighting.ClockTime = fullbrightSaved.ClockTime
            end
            if fullbrightSaved.FogEnd then
                Lighting.FogEnd = fullbrightSaved.FogEnd
            end
            if fullbrightSaved.FogStart then
                Lighting.FogStart = fullbrightSaved.FogStart
            end
            if fullbrightSaved.GlobalShadows ~= nil then
...
tpBtn.MouseButton1Click:Connect(
    function()
        if teleportSelected then
            pcall(
                function()
                    teleportToPlayer(teleportSelected)
                end
            )
        end
    end
...
print("[vanz] Slim menu + Silent Aim + Radius Parry")
