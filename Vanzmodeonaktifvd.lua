...
-- BLOCK 3: FITUR (Wallhack, ESP, Aimbot, Parry, Silent, Teleport, Waypoint, dll)
do
    local Players = _G.vanz.Players
    ...
    _G.vanz.applyWallhack = applyWallhack
    _G.vanz.restoreWallhack = restoreWallhack
    ...
    _G.vanz.setFlyEnabled = function(s) flyEnabled = s end
    _G.vanz.getFlyEnabled = function() return flyEnabled end

    _G.vanz.applySpeed = applySpeed
    ...
    _G.vanz.getCurrentSpeed = function() return currentSpeed end

    _G.vanz.startFly = startFly
    _G.vanz.stopFly = stopFly
    _G.vanz.getCurrentSpeedFn = getCurrentSpeed

    _G.vanz.setESP = function(s) espEnabled = s if s then refreshESP() else clearAllESP() end end
    _G.vanz.refreshESP = refreshESP
    _G.vanz.clearAllESP = clearAllESP
    ...
    _G.vanz.getFullbrightEnabled = function() return fullbrightEnabled end
    ...
    _G.vanz.setTeleportV2Enabled = function() end
end

-- BLOCK 4: AUTO PARRY + SILENT AIM
do
    local Players = _G.vanz.Players
    local RunService = _G.vanz.RunService
    local Workspace = _G.vanz.Workspace
    ...
    local function tryInstantParry(char)
        ...
        if now - last < parryDebounce then return end
    ...
    local function createParryRing()
        ...
        if parryRingInner then
            for _, p in ipairs(parryRingInner) do pcall(function() p:Destroy() end) end
        end
        parryRingOuter = {}
        parryRingInner = {}
        local myHrp = getMyHrp()
    ...
    local function startAutoParryLoop()
        if parryRingConn then parryRingConn:Disconnect() end
        parryRingConn = RunService.Heartbeat:Connect(function()
    ...
    _G.vanz.setParryRingEnabled = function(s) parryRingEnabled = s if not s then destroyParryRing() else createParryRing() end end
    ...
    pcall(function() silentAimMouse = LP:GetMouse() end)
    ...
    local function isSilentTargetValid()
        if not silentAimTarget then return false end
        local char = silentAimTarget.Character
    ...
    local function isTargetInRange()
        if not isSilentTargetValid() then return false end
        local myHrp = getMyHrp()
        if not myHrp then return false end
    ...
    local function updateSilentCamera()
        ...
        if not isTargetInRange() then return end
    ...
    local function stopSilentAim()
        ...
        if silentAimLastCFrame then
            local cam = Workspace.CurrentCamera
            if cam then cam.CFrame = silentAimLastCFrame end
            silentAimLastCFrame = nil
        end
    end
    ...
    Players.PlayerRemoving:Connect(function(plr)
        if silentAimTarget == plr then silentAimTarget = nil end
        task.wait(0.1)
        refreshSilentTargetList()
    end)
    ...
    if silentAimMouse then
        silentAimMouse.Button1Down:Connect(function()
            if silentAimEnabled then spoofMouse() end
        end)
        silentAimMouse.Button1Up:Connect(function()
            if silentAimOriginalHit and silentAimMouse then
                pcall(function()
                    silentAimMouse.Hit = silentAimOriginalHit
                end)
    ...
    _G.vanz.startSilentAim = startSilentAim
    _G.vanz.stopSilentAim = stopSilentAim
    _G.vanz.setSilentMode = function(m) silentAimMode = m end
    _G.vanz.setSilentTarget = function(p) silentAimTarget = p end
    _G.vanz.getSilentTarget = function() return silentAimTarget end
    _G.vanz.setSilentListFrame = function(f) silentListFrame = f end
    _G.vanz.refreshSilentTargetList = refreshSilentTargetList

    task.wait(0.5)
    refreshGuiMobCache()
    refreshSilentTargetList()
    _G.vanz.refreshTeleportList()
    startAutoParryLoop()
end

-- BLOCK 5: TAB BUILDERS
do
    local Players = _G.vanz.Players
    local UIS = _G.vanz.UIS
    local T = _G.vanz.T
    ...
    makeButton("HOP RAMAI", function()
        _G.vanz.hopToServer("crowded")
    end, ub)
    makeButton("HOP SEPI", function()
        _G.vanz.hopToServer("empty")
    end, ub)
    ...
    makeToggle("Silent Aim", false, function(state)
        if state then _G.vanz.startSilentAim() else _G.vanz.stopSilentAim() end
    end, sb)
    ...
    local modeOptions = {{"camera", "Camera"}, {"mouse", "Mouse"}, {"hybrid", "Hybrid"}}
    local modeButtons = {}
    local function setSilentMode(idx)
        _G.vanz.setSilentMode(modeOptions[idx][1])
    ...
    for i, opt in ipairs(modeOptions) do
        local btn = Instance.new("TextButton", modeRow1)
    ...
    makeInfo("Camera: override kamera. Mouse: override Mouse.Hit. Hybrid: dua-duanya.", sb)
    ...
    makeButton("REFRESH TARGET LIST", function()
        _G.vanz.refreshSilentTargetList()
    end, sb)
end
