...
-- BLOCK 3: CORE FEATURES (Wallhack, Speed, Fly, ESP, Aimbot, Anti-Ragdoll, Fullbright, Server Hop, Waypoint, Teleport)
do
    local Players = _G.vanz.Players
    ...
    local function createESPTag(plr)
        ...
        espData[plr] = {
            box = box, hl = hl, bb = bb, line = line,
            nameLbl = nl, hf = hf, ht = ht,
            lastColor = col, lastChar = char,
        }
    end
    ...
    Players.PlayerAdded:Connect(function() task.wait(0.5) refreshTeleportList() end)
    Players.PlayerRemoving:Connect(function(plr)
        if teleportSelected == plr then teleportSelected = nil end
        task.wait(0.1)
        refreshTeleportList()
    end)
    task.spawn(function()
        while true do task.wait(0.5) refreshTeleportList() end
    end)
...
do
    ...
    local function pollKillerAndParry()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP then
                local char = getKillerChar(plr)
                if char and isKillerInRange(char, myHrp) then tryInstantParry(char) end
            end
        end
    end
...
-- BLOCK 5: SILENT AIM (6 MEKANISME TERPISAH)
do
    local Players = _G.vanz.Players
    local RunService = _G.vanz.RunService
    local Workspace = _G.vanz.Workspace
    ...
    local function getTargetPart()
        ...
        elseif silentBodyPart == "Kaki" then return char:FindFirstChild("LeftFoot") or char:FindFirstChild("RightFoot") or char:FindFirstChild("LeftLowerLeg") or char:FindFirstChild("RightLowerLeg")
        end
    ...
    local function getTargetPos()
        ...
        elseif silentBodyPart == "Paha" and p.Name == "LowerTorso" then
            return p.Position - Vector3.new(0, 1, 0)
        elseif silentBodyPart == "Kaki" and p.Name:find("Leg") then
            return p.Position - Vector3.new(0, 0.5, 0)
        end
        return p.Position
    end
    ...
    local function mouseEnable()
        ...
        if mouse.Button1Down:GetConnectionCount() > 0 then return end
    ...
    -- MEKANISME 3: CAMERA RAY
    local function cameraRayEnable()
        cameraRayOn = true
    end

    local function cameraRayDisable()
        cameraRayOn = false
    end

    -- MEKANISME 4: WORKSPACE RAYCAST
    local function wsRaycastEnable()
        wsRaycastOn = true
    end

    local function wsRaycastDisable()
        wsRaycastOn = false
    end

    -- MEKANISME 5: FIND PART ON RAY
    local function findPartOnRayEnable()
        findPartOnRayOn = true
    end

    local function findPartOnRayDisable()
        findPartOnRayOn = false
    end

    -- MEKANISME 6: REMOTE INJECT (nama call hook terpisah & isolated)
    local function remoteInjectEnable()
        remoteInjectOn = true
    end

    local function remoteInjectDisable()
        remoteInjectOn = false
    end
    ...
    local function refreshSilentList()
        ...
        for _, row in pairs(silentTargetRows) do pcall(function() row:Destroy() end) end
        silentTargetRows = {}
    ...
    Players.PlayerAdded:Connect(function() task.wait(0.5) refreshSilentList() end)
    Players.PlayerRemoving:Connect(function(plr)
    ...
    task.spawn(function()
        while true do task.wait(1) refreshSilentList() end
    end)
    ...
    _G.vanz.silentHitboxOn = hitboxEnable
    _G.vanz.silentHitboxOff = hitboxDisable
    _G.vanz.silentMouseOn = mouseEnable
    _G.vanz.silentMouseOff = mouseDisable
    _G.vanz.silentCameraRayOn = cameraRayEnable
    _G.vanz.silentCameraRayOff = cameraRayDisable
    _G.vanz.silentWsRaycastOn = wsRaycastEnable
    _G.vanz.silentWsRaycastOff = wsRaycastDisable
    _G.vanz.silentFindPartOnRayOn = findPartOnRayEnable
    _G.vanz.silentFindPartOnRayOff = findPartOnRayDisable
    _G.vanz.silentRemoteInjectOn = remoteInjectEnable
    _G.vanz.silentRemoteInjectOff = remoteInjectDisable
    _G.vanz.getSilentTargetPos = getTargetPos
    _G.vanz.isSilentTargetValid = isTargetValid

    task.wait(0.5)
    refreshSilentList()
end

-- BLOCK 6: METATABLE HOOK (ISOLATED — DIPISAH SENDIRI)
do
    local Workspace = _G.vanz.Workspace
    ...
    if mt and mt.__namecall then
        ...
        local ok = pcall(function()
            mt.__namecall = newcclosure(function(self, ...)
                ...
                if _G.vanz.isSilentTargetValid and _G.vanz.isSilentTargetValid() then
                    local pos = _G.vanz.getSilentTargetPos and _G.vanz.getSilentTargetPos()
                    if pos then
                        if _G.vanz.silentRemoteInjectOn and _G.vanz.silentRemoteInjectOn() then
                        ...
                        if self == Workspace and method == "Raycast" then
                            if _G.vanz.silentWsRaycastOn and _G.vanz.silentWsRaycastOn() then
                                local args = {...}
                        ...
                        if self == Workspace and (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist") then
                            if _G.vanz.silentFindPartOnRayOn and _G.vanz.silentFindPartOnRayOn() then
                                local args = {...}
                                for i, arg in ipairs(args) do
...
do
    ...
    makeToggle("Auto Parry", false, function(state) _G.vanz.setAutoParry(state) end, cb)
    makeNum("Parry Range", 20, function(val) _G.vanz.setParryRange(val) end, cb)
    makeToggle("Ring Visual", true, function(state) _G.vanz.setParryRingEnabled(state) end, cb)

    -- VISUAL
    makeInfo("Visual: ESP + Fullbright", vb)
    makeToggle("ESP", false, function(state) _G.vanz.setESP(state) end, vb)
    ...
    makeSlider("Brightness", 0, 10, 3, function(val) _G.vanz.setFullbrightLevel(val) end, vb)

    -- MOVEMENT
    makeInfo("Movement: Wallhack, Speed, Jump, Fly", mb)
    makeToggle("Wallhack", false, function(state) _G.vanz.setWallhack(state) end, mb)
    makeToggle("Speed Hack", false, function(state) _G.vanz.setSpeed(state) end, mb)
    makeNum("Speed", 100, function(num) _G.vanz.setCurrentSpeed(num) end, mb)
    local infJump = false
    ...
    makeToggle("Infinite Jump", false, function(state) infJump = state end, mb)
    ...
    makeInfo("Waypoint: tandai posisi, teleport balik", tpb)
    makeToggle("Waypoint (Mark)", false, function(state)
        if state then _G.vanz.markWaypoint() else _G.vanz.clearWaypoint() end
    end, tpb)
    makeButton("TELEPORT KE WAYPOINT", function() _G.vanz.teleportToWaypoint() end, tpb)
    ...
    makeToggle("Team Safe (Aimbot)", true, function(state) _G.vanz.setAimbotTeamSafe(state) end, ab)
    makeSlider("POV", 5, 360, 60, function(val) _G.vanz.setAimbotPOV(val) end, ab)
    ...
    -- SILENT AIM (TERPISAH PER MEKANISME)
    makeInfo("Silent Aim: pilih mekanisme dulu sebelum ON. Coba 1 per 1 kalau takut FC.", sb)
    makeInfo("Mekanisme 1: Hitbox Expand (paling aman)", sb)
    makeToggle("Hitbox Expand", false, function(state)
        if state then _G.vanz.silentHitboxOn() else _G.vanz.silentHitboxOff() end
    end, sb)
    makeInfo("Mekanisme 2: Mouse Spoof", sb)
    makeToggle("Mouse Spoof", false, function(state)
        if state then _G.vanz.silentMouseOn() else _G.vanz.silentMouseOff() end
    end, sb)
    makeInfo("Mekanisme 3: Camera Ray Redirect (butuh metatable)", sb)
    makeToggle("Camera Ray", false, function(state)
        if state then _G.vanz.silentCameraRayOn() else _G.vanz.silentCameraRayOff() end
    end, sb)
    makeInfo("Mekanisme 4: Workspace Raycast Redirect (butuh metatable)", sb)
    makeToggle("Workspace Raycast", false, function(state)
        if state then _G.vanz.silentWsRaycastOn() else _G.vanz.silentWsRaycastOff() end
    end, sb)
    makeInfo("Mekanisme 5: FindPartOnRay Redirect (butuh metatable)", sb)
    makeToggle("FindPartOnRay", false, function(state)
        if state then _G.vanz.silentFindPartOnRayOn() else _G.vanz.silentFindPartOnRayOff() end
    end, sb)
    makeInfo("Mekanisme 6: Remote Arg Inject (paling risiko FC)", sb)
    makeToggle("Remote Inject", false, function(state)
        if state then _G.vanz.silentRemoteInjectOn() else _G.vanz.silentRemoteInjectOff() end
    end, sb)

    makeInfo("Hit Target (Kepala/Leher/Badan/Paha/Kaki):", sb)
    ...
    silentScroll.ScrollBarImageColor3 = T.accent    silentScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
...
print("[vanz] Slim menu + Parry + Silent Aim (mekanisme terpisah)")
