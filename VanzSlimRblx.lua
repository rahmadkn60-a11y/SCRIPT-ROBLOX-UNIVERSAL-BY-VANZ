...
local function makeSliderFloat(name, mn, mx, default, step, cb, parentOverride)
    local p = parentOverride or body
    local r = Instance.new("Frame", p)
    r.Size = UDim2.new(1, 0, 0, 54)
    r.BackgroundColor3 = T.row
    ...
    lb.Position = UDim2.new(0, 12, 0, 4)
    lb.BackgroundTransparency = 1
    lb.Text = name .. ": " .. string.format("%.2f", default)
    lb.TextColor3 = T.text
    lb.TextSize = 13
    lb.Font = Enum.Font.Gotham
    lb.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local function upd(inp)
        ...
        fill.Size = UDim2.new(rel, 0, 1, 0)
        lb.Text = name .. ": " .. string.format("%.2f", v)
        if cb then
            pcall(cb, v)
        end
    end
...
local parryLeadTime = 0.15
local parryHitTimeRatio = 0.5
local parryDebounce = 0.4

local attackTracks = {}
local parryDebounceMap = {}
local autoParryLastFire = 0
local autoParryFireCooldown = 0.05
            ...
            if isParryAttackAnimation(aid) then
                task.spawn(
                    function()
                        ...
                        attackTracks[char][track] = entry
                        task.delay(
                            math.max(length * 1.5, 2),
                            function()
                                if attackTracks[char] then
                                    attackTracks[char][track] = nil
                                end
                            end
                        )
                    end
...
local function getDebounceKey(char, track)
    return tostring(char:GetDebugId()) .. "_" .. tostring(track)
end

local function tryPredictiveParry()
    ...
    local now = tick()
    for char, tracks in pairs(attackTracks) do
        if char and char.Parent then
            for track, entry in pairs(tracks) do
                if not entry.fired and track.IsPlaying then
                    local elapsed = now - entry.startTime
                    local length = entry.length or 1
                    local hitTime = length * parryHitTimeRatio
                    local fireTime = hitTime - parryLeadTime
                    if elapsed >= fireTime and elapsed <= hitTime + 0.1 then
                        if isKillerInParryRange(char, myHrp) then
                            local key = getDebounceKey(char, track)
                            local lastFire = parryDebounceMap[key] or 0
                            if now - lastFire >= parryDebounce then
...
makeInfo("Combat: Anti-Ragdoll + Auto Parry (Predictive)", cb)
makeToggle(
    "Anti Ragdoll",
    false,
    function(state)
...
print("[vanz] Slim menu + Predictive Auto Parry")
